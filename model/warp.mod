...
maximize Profit:
  ...
  - (sum{m in M, p in P} mach_op_min[m] * (proc_time[p,m] * x[p])/60) #cost to make shoes using machine
  ...
  - (10 * sum{p in P} max(0,D[p]-x[p])) #unmet demand
  - (sum{w in W} wh_op_cost[w] * y[w]); #cost of warehouse

subject to Machine_Capacity{m in M}:
    sum{p in P} proc_time[p,m] * x[p] <= machine_capacity_sec[m]; #time has to be less than 12 hours a day 28 days a month, averaged out
...
subject to WarehouseCap: #the total shoes is less than all operating warehouse
    sum{p in P} x[p] <= sum{w in W} warehouse_cap[w] * w[w];
